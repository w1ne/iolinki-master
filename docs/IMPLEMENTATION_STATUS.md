...
| Events | Partial | [`include/iolinki_master/master.h`](../include/iolinki_master/master.h), [`src/master_isdu.c`](../src/master_isdu.c), [`tests/test_master_isdu_public.c`](../tests/test_master_isdu_public.c); on-wire event trigger and `iolink_master_read_event_details` returning code 0x8CA0 against real device firmware in `labwired-core` test `world_station_services.rs :: master_services_isdu_pdout_event_ds_all_pass_on_wire` (this repo's `labwired-real-firmware-model` CI job) | Validate event flows against real devices. |
