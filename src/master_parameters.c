...
{
    uint8_t time_base = (uint8_t) ((octet >> IOLINK_MASTER_MIN_CYCLE_BASE_SHIFT) &
                                   IOLINK_MASTER_MIN_CYCLE_BASE_MASK);
...
static bool iolink_master_mseq_type_from_capability_code(uint8_t code, bool isdu_supported,
                                               ...
                                               iolink_master_device_info_t* info)
{
    ...
    info->operate_mseq_code = (uint8_t) ((page[IOLINK_MASTER_DPP1_OFF_MSEQ_CAPABILITY] >>
                                         ...
                                         IOLINK_MASTER_MSEQ_CAP_OPERATE_MASK);
    info->preoperate_mseq_code = (uint8_t) ((page[IOLINK_MASTER_DPP1_OFF_MSEQ_CAPABILITY] >>
                                             IOLINK_MASTER_MSEQ_CAP_PREOP_SHIFT) &
                                            IOLINK_MASTER_MSEQ_CAP_PREOP_MASK);
    ...
    info->vendor_id = (uint16_t) (((uint16_t) page[IOLINK_MASTER_DPP1_OFF_VENDOR_ID_HI] << 8U) |
                                                 ...
                                                 iolink_master_config_t* config)
{
    ...
    if (!iolink_master_mseq_type_from_capability_code(info->operate_mseq_code, info->isdu_supported,
                                                      info->pd_in_len, info->pd_out_len,
                                                      &m_seq_type)) {
        return IOLINK_MASTER_PARAM_ERR_M_SEQUENCE;
    }
